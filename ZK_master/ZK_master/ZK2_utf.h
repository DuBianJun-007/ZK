...
private:
	Next2_utf* next;
	Next2_utf* ZKNext(const char* PStr, int PLen, const char* charSet);
	int NextStart(int Index[], int Distance[], int Sequence[], int& TStart, int num, int t);
	void FNext(const char* PStr, const  char* characters, int e, int start, Next2_utf* next);
public:
	ZK2_utf();
	ZK2_utf(const char* TStr, const char* PStr, int TLen, int PLen, const char* charSet = NULL) {
